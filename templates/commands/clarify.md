...
2. 現在の spec ファイルを読み込む。この分類法を使用して構造化された曖昧さ＆カバレッジスキャンを実行する。各カテゴリについて、ステータスをマークする: Clear (明確) / Partial (部分的) / Missing (欠落)。優先順位付けに使用される内部カバレッジマップを生成する (質問が行われない場合を除き、生マップを出力しない)。
...
4. 順次質問ループ (対話型):
    ...
    - 多肢選択質問の場合:
       ...
       - フォーマット: `**推奨:** Option [X] - <理由付け>`
       - その後、すべてのオプションをMarkdownテーブルとしてレンダリングする (Option, Description の2列)。短い回答 (<=5 words) もオプションに含める。
...
5. 各**承認された回答**後の統合 (増分更新アプローチ):
    ...
    - その後、直ちに明確化を最も適切なセクションに適用する:
       ...
       - 用語の競合 → spec全体で用語を正規化; 必要な場合のみ `(以前は "X" と呼ばれていた)` を一度追加して元の用語を保持。
...
8. 完了報告 (質問ループ終了後または早期終了後):
   ...
   - 更新された spec へのパス。
   - 更新されたセクション (名前をリスト)。
   - カバレッジサマリーテーブル。各分類カテゴリのステータス: Resolved (解決済み - Partial/Missingだったが対処された), Deferred (保留 - 質問クォータ超過または計画に適している), Clear (明確 - すでに十分), Outstanding (未解決 - まだPartial/Missingだが低影響)。
   - Outstanding または Deferred が残っている場合、`/speckit.plan` に進むか、計画後に再度 `/speckit.clarify` を実行するか推奨する。
   - 提案される次のコマンド。
